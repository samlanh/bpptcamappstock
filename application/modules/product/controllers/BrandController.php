...
public function init()
    ...
    public function indexAction()
    {
		...
		$columns=array("BRAND_NAME","STATUS","REMRK");
		$link=array(
				'module'=>'product','controller'=>'brand','action'=>'edit',
		);
		$list = new Application_Form_Frmlist();
		$this->view->list=$list->getCheckList(0, $columns, $rows, array('name'=>$link,'parent_id'=>$link,'barcode'=>$link,
				'branch'=>$link));
	...
	public function addAction()
	{
		...
		if($this->getRequest()->isPost()) {
			...
			if(isset($data['btnsavenew'])){
				Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS", '/product/brand/add');
			}else {
				Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS", '/product/brand');
			}
		}
		$formFilter = new Product_Form_FrmBrand();
