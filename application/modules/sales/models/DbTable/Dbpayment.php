...
// 					$recipt_paid = $data['paid_amount'.$i];//well
					$balance=0;
					$compelted=1;
				}else{
					$paided = $data['paid'];
// 					$recipt_paid = $paid;//($data['paid_amount'.$i]- abs($paid));
...
// 						'discount'  => 	$data['discount'.$i],
				...
				if(!empty($rsinvoice)){
					$data_invoice = array(
								'paid_amount'=>$rsinvoice['paid_amount']+$paided,
								'discount_after'  => 	0,
								'paid_after'	  => 	$paided,
								'balance_after'	  => 	$balance,
								'sub_total_after' =>    $balance,
								'is_fullpaid'	  => 	$compelted,
								);
