...
public  function getDailywork($search=null){
  ...
  $sql = "SELECT id, work ,
	(SELECT cust_name FROM `tb_customer` WHERE tb_customer.id=tb_dailywork.customer_id LIMIT 1 ) AS customer_name,
	...
	(SELECT fullname  FROM `tb_acl_user`
	WHERE `tb_acl_user`.`user_id`=`tb_dailywork`.`user`) 
	AS `user`
  ...
  return $db->fetchAll($sql.$where);
	...
	public  function getDailyworkProcess($search=null){
		...
		date ,
		(SELECT fullname  FROM `tb_acl_user`
		WHERE `tb_acl_user`.`user_id`=`tb_dailyworkprocess`.`user`)
		AS `user`,
		description ,
		(SELECT name_en FROM `tb_view` WHERE type=11 AND key_code=work_status LIMIT 1) as work_status
		FROM `tb_dailyworkprocess` WHERE work_status=2 ";
		$where = '';
		if(!empty($search["adv_search"])){
			$s_where=array();
			$s_search = addslashes(trim($search['adv_search']));
