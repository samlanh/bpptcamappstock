<?php
class Dailywork_Model_DbTable_DbWorkcomplete extends Zend_Db_Table_Abstract
{
    protected $_name = 'tb_workcomplete';
    ...
    public function add($_data){
	  $this->_name='tb_workcomplete';
	  $_arr = array(
	    'workcompleted'=>$_data['workcomplete'],
	    'noteandidea'=>$_data['idea'],
	    );
 ...
 function getUser(){
 	...
 	$sql ="SELECT u.user_id AS id,u.`fullname` as first_name  FROM `tb_acl_user` AS u where u.status=1 ";
	$where = '';
 	if($result['level']==1 OR $result['level']==2){
 		
 	}
 	else{
 		$sql.= " AND  u.user_id=".$result['user_id'];
 		//return $result;
 	}
 ...
 public function updateworkcompleteById($_data){
 //print_r($_data); exit();
 	$_arr = array(
 ...
 public function getworkcompleteById($id)
 {
 	$db = $this->getAdapter();
 	$sql = "SELECT id , workcompleted , noteandidea  FROM tb_workcomplete WHERE id = $id";
...
public  function getworkcomplete($search=null){
  $db = $this->getAdapter();
  ...
  if(!empty($search["adv_search"])){
  	...
  	$s_where[]= " noteandidea LIKE '%{$s_search}%'";
  	$where.=' AND ('.implode(' OR ', $s_where).')';
  }
  if(!empty($search["workcompleted"])){
  	$where.=' AND id='.$search["workcompleted"];
  }
  //echo $sql.$where;
