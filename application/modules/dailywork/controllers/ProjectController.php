<?php
class Dailywork_ProjectController extends Zend_Controller_Action
{
public function init()
    ...
    {
    	$db =new Dailywork_Model_DbTable_DbProject();
    	...
    	$this->view->list=$list->getCheckList(0, $columns, $rows, array('projectname'=>$link));
 		$db =new Dailywork_Model_DbTable_DbProject();
 		$dd=$db->getProject();
       $this->view->rst=$dd;
	}
	public function addAction()
	{
		if ($this->getRequest()->isPost()){
			try {
				...
				$project = new Dailywork_Model_DbTable_DbProject();
				$project->add($data);
				if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/project/index");
					
				}else {
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/project/add");
				}
			}
			catch (Exception $e){
		...
		$fm=new Dailywork_Form_FrmProject();
		$frm_project=$fm->add();
		Application_Model_Decorator::removeAllDecorator($frm_project);
		$this->view->frm_project= $frm_project;
		
	}// Add Product 
	public function editAction(){
		$id=$this->getRequest()->getParam('id');
    	$db = new Dailywork_Model_DbTable_DbProject();
    	$dd= $this->view->row_rs=$db->getprojectById($id);
    	if ($this->getRequest()->isPost()){
    		$data = $this->getRequest()->getPost();
    		$data['id']=$id;
    		$product = new Dailywork_Model_DbTable_DbProject();
    	 	$product->updateprojectById($data);
    	 	Application_Form_FrmMessage::message("INSERT_SUCESS");
    	 	Application_Form_FrmMessage::redirectUrl("/dailywork/project");
    	}
    	$fm=new Dailywork_Form_FrmProject();
    	$frm_project=$fm->add($dd);
    	Application_Model_Decorator::removeAllDecorator($frm_project);
    	$this->view->frm_project= $frm_project;
   }
