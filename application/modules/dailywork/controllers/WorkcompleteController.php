<?php
class Dailywork_WorkcompleteController extends Zend_Controller_Action
{
public function init()
    ...
    public function indexAction()
    {
    	...
    	$rows = $db->getDailyworkCompleted($search);
    	//$this->view->row_rs = $db->getworkcomplete();
    	//print_r($db); exit();
    	$this->view->row_rs = $rows;
    	$list = new Application_Form_Frmlist();
    	$columns=array("WORK_TITLE" , "CUSTOMER_NAME","TASK","DATE" , "BY_USER","DESCRIPTION","STATUS");
    	$link=array(
    			'module'=>'dailywork','controller'=>'workcomplete','action'=>'edit',
    	);
	...
	public function addAction()
	{
		if ($this->getRequest()->isPost()){
			try {
				...
				$dailywork->addworkcompleted($data);
				if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/workprocess");
				}else{
					Application_Form_FrmMessage::Sucessfull("Complete", "/workprocess/add");
				}
			}
			catch (Exception $e){
...
// 		$db = new Product_Model_DbTable_Dbproduct(); //creat object store data
// 		$this->view->rst = $db->selectproduct(); //call fontion
// 	}// select Produuct 
	public function editAction(){
    	...
    	if ($this->getRequest()->isPost()){
    		...
    		$product = new Dailywork_Model_DbTable_DbDailywork();
    	 	$product->editworkcompleted($data);
    		Application_Form_FrmMessage::message("EDIT_SUCESS");
    		Application_Form_FrmMessage::redirectUrl("/dailywork/workcomplete");
    	}
    	if(empty($id)){
    		Application_Form_FrmMessage::Sucessfull("NO_RECORD_EDIT", "/dailywork/workcomplete");
    	}
    	$fm=new Dailywork_Form_Frmworkprocess();
