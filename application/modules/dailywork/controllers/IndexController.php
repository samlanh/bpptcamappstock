...
public function init()
    ...
    public function indexAction()
    {
    	...
    	$columns=array("TASK_TITLE", "CUSTOMER_NAME","TASK","DATE" , "USER_ID","DESCRIPTION","STATUS");
    	$link=array(
    			'module'=>'dailywork','controller'=>'index','action'=>'edit',
    	);
    	$this->view->list=$list->getCheckList(0, $columns, $rows, array('projectname'=>$link,'customer_name'=>$link,'user'=>$link));
 		$this->view->rst=$rows;
		...
		if ($this->getRequest()->isPost()){
			try {
				...
				if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/index");
				}else{
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/add");
				}
			}
			catch (Exception $e){
