...
class Purchase_Model_DbTable_DbPurchaseReceive extends Zend_Db_Table_Abstract
{	
	function getAllPurchaseOrder($search){//new
		...
		sub_total_pro,sub_total_jobtype,
...
// 					}
	...
	public function updatePurchaseOrder($data)
		{
			...
			try{
				...
				unset($info_purchase_order);
             //deelect tb_purchase_order_item 
				$this->_name='tb_purchase_order_item';
	...
	public function getPurchaseDetailById($id){//just new
		$db=$this->getAdapter();
		$sql = " SELECT * FROM `tb_purchase_order_item` AS pt 
                WHERE pt.purchase_id=$id AND pt.type=1";
	...
	public function getPurchaseDetailJobTypById($id){//just new
		$db=$this->getAdapter();
		$sql = " SELECT * FROM `tb_purchase_order_item` AS pt
		WHERE pt.purchase_id=$id AND pt.type=0";
		$rows=$db->fetchAll($sql);
		return $rows;
	}
